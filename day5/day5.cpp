//
// Created by olive on 05/12/2024.
//

#include "day5.h"
#include <unordered_map>
#include <unordered_set>
#include <vector>
#include <sstream>
#include <iostream>
#include <ranges>
#include <algorithm>
#include <numeric>

static auto read(std::ifstream &file) {
  file.clear();
  file.seekg(std::ios::beg);
  std::string line;
  std::unordered_map<int, std::unordered_set<int>> rules{};
  while (file.peek() != '\n' && std::getline(file, line)) {
    std::stringstream ss{line};
    int key, value;
    char bar;
    ss >> key >> bar >> value;
    rules[key].insert(value);
  }
  file.get();
  std::vector<std::vector<int>> books;
  int index{0};
  char comma;
  while (std::getline(file, line)) {
    std::stringstream ss{line};
    books.emplace_back();
    for (int i = 0; ss >> i;) {
      books[index].push_back(i);
      ss >> comma;
    }
    ++index;
  }
  return std::tuple{rules, books};
}

std::uint64_t day5::part1() {
  auto [rules, books] = read(file);
  return *std::ranges::fold_left_first(books
                                           | std::views::filter([&](auto &book) {
                                             return std::ranges::all_of(
                                                 book | std::views::adjacent_transform<2>([&](auto first,
                                                                                              auto second) {
                                                   return rules[first].contains(second);
                                                 }),
                                                 std::identity{});
                                           })
                                           | std::views::transform([](const auto &book) {
                                             return book[book.size() / 2];
                                           }), std::plus{});
}
std::uint64_t day5::part2() {
  auto [rules, books] = read(file);
  return *std::ranges::fold_left_first(books
                                           | std::views::filter([&](auto &book) {
                                             return !std::ranges::all_of(
                                                 book | std::views::adjacent_transform<2>([&](auto first,
                                                                                              auto second) {
                                                   return rules[first].contains(second);
                                                 }),
                                                 std::identity{});
                                           })
                                           | std::views::transform([&](auto &el) {
                                             std::ranges::sort(el, [&](auto first, auto second) {
                                               return rules[first].contains(second);
                                             });
                                             return el[el.size() / 2];
                                           }), std::plus{});
}
